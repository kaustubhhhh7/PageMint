                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                    {programs.map((program) => (
                        <div
                            ...
                            className={`group relative p-8 bg-slate-50 border-2 rounded-3xl cursor-pointer transition-all duration-300 hover:-translate-y-2
                            ${selectedProgram === program.id
                                    ? 'border-orange-500 shadow-xl'
                                    : 'border-transparent hover:border-slate-200 hover:shadow-lg'}`}
                        >
                            <div className="flex justify-between items-start mb-6">
                                <span className="text-4xl bg-white w-16 h-16 flex items-center justify-center rounded-2xl shadow-sm border border-slate-100 group-hover:scale-110 transition-transform duration-300">
                                    {program.icon}
                                </span>
                                <span className={`px-3 py-1 text-xs font-bold uppercase rounded-full ${program.level === 'Advanced' ? 'bg-red-100 text-red-600' :
                                        program.level === 'Intermediate' ? 'bg-orange-100 text-orange-600' :
                                            'bg-green-100 text-green-600'
                                    }`}>
                                    {program.level}
                                </span>
                            </div>
                            <h3 className="text-2xl font-bold text-slate-900 mb-2 uppercase italic">{program.title}</h3>
                            <p className="text-slate-600 mb-4">{program.desc}</p>

                            <div className="flex items-center text-orange-600 font-bold uppercase text-sm group-hover:gap-2 transition-all">
                        ...
                        <div className="bg-slate-900 rounded-3xl p-8 md:p-12 text-white relative overflow-hidden">
                            ...
                            <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8">
                                ...
                                <div className="flex flex-col justify-center items-start md:items-end">
                                    <button className="w-full md:w-auto px-8 py-4 bg-orange-500 text-white font-bold uppercase tracking-wider hover:bg-orange-600 transition-all hover:scale-105 shadow-xl shadow-orange-500/30 skew-x-[-10deg]">

...
const HighlightCard = ({ number, title, desc, stat, icon: Icon, color, bgColor, delay }) => {
    ...
    return (
        <div
            ...
            className={`group relative ${bgColor} rounded-[2rem] p-6 lg:p-10 overflow-hidden border border-white/50 hover:border-indigo-200 transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1`}
        >
            <div className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br ${color} opacity-10 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/2 group-hover:opacity-20 transition-opacity`} />

            <div className="relative z-10 grid lg:grid-cols-2 gap-6 lg:gap-8 h-full items-center">
                <div className="flex flex-col justify-between h-full">
                    <div>
                        <span className="text-5xl font-black text-slate-900/10 mb-4 block select-none group-hover:text-slate-900/20 transition-colors">
                            {number}
                        </span>
                        <div className="mb-4 inline-flex p-3 rounded-2xl bg-white shadow-sm border border-slate-100/50">
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-200/50">
                        ...
                        <p className="text-xl font-bold text-slate-900 mt-1">{stat}</p>
                    </div>
                </div>

                {/* Visual Panel with Parallax */}
                <div className="visual-panel h-56 lg:h-full min-h-[220px] bg-white rounded-2xl relative overflow-hidden transition-transform duration-200 ease-out border border-slate-100/50 shadow-sm">
